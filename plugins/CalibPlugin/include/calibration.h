...
class Operator {
   public:
    ...
    void fitPlane(CloudPtr source, CloudPtr &result, float &a, float &b,
    ...
    inline void setDebugFolder(const std::string &name) {
        debug_folder_ = name;
        if (debug_folder_.size()) {
            if (debug_folder_.substr(debug_folder_.size() - 1, 1) !=
                std::string("/")) {
